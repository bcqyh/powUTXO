...
func NewCoinbaseTx(miner string, data string) *Transaction {
	...
	//在后面的程序中，需要识别一个交易是否为coinbase，所以需要设置一些特殊的值，用于判断
...
func NewTransaction(from, to string, amount float64, bc *BlockChain) *Transaction {
	...
	//付款人再创建交易时，已经得到了所有引用的output的详细信息。
	//但是不去使用，因为在矿工校验的时候，矿工是没有这部分信息的，矿工需要遍历账本找到所有引用交易
	//为了统一操作，所以再次查询一次，进行签名。
